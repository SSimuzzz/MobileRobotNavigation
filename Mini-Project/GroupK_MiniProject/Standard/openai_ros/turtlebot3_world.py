...
class TurtleBot3WorldEnv(turtlebot3_env.TurtleBot3Env):
    ...
    def _heading_to_goal(self, x, y, yaw):
        ...
        gx, gy = self.goal_xy
        goal_bearing = math.atan2(gy - y, gx - x)   # absoluta angle to the goal
    ...
    def _get_obs(self):
        ...
        self.scan_front_observations(discretized_observations)
        ...
        # normalizations:
        # - dist: divide by a plausible maximum distance from the world
        # - heading: in [-pi, pi]
    ...
    def _compute_reward(self, observations, done):
        ...
        # --- 2. Factorized reward calculation ---

        # A. Progress Reward: based on difference between previous distance and current distance to goal
        ...
        if done:
            if self.reached_goal:
                ...
                # If episode done, but it is no goal reached or timeout --> collision
                r_terminal = self.terminal_crash

        # E. Time Rewards
        r_cumm_time = self.steps
    ...
    def scan_front_observations(self, discretized_ranges):
        self.front_ranges = []
        self.front_angles = []
